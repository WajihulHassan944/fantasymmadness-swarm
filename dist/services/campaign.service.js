import { AUTOMATION_DEFINITIONS, automationByKey } from '../automations/definitions.js';
import { createCampaignSchema, listCampaignsQuerySchema, } from '../contracts/campaign.js';
...
export function listCampaignPacks() {
    return CAMPAIGN_PACKS;
}
export async function createCampaign(rawInput) {
    const input = createCampaignSchema.parse(rawInput);
    const pack = CAMPAIGN_PACKS.find((item) => item.campaignType === input.campaignType);
    if (!pack)
        throw new AppError(400, 'CAMPAIGN_PACK_NOT_FOUND', `Unsupported campaign type: ${input.campaignType}`);
...
export async function listCampaigns(rawQuery) {
    const query = listCampaignsQuerySchema.parse(rawQuery ?? {});
    const filter = {};
    if (query.status)
        filter.status = query.status;
    if (query.campaignType)
        filter.campaignType = query.campaignType;
    if (query.vertical)
...
async function upsertOrCreateCampaign(input, pack, title, vertical, sport, sourceEntity) {
...
function selectDefinitions(pack, options) {
    ...
    const sectionFiltered = options.includeAll || !options.sections?.length
        ? definitions
        : definitions.filter((definition) => options.sections?.includes(definition.category));
    const compatible = sectionFiltered.filter((definition) => definition.vertical === options.vertical || definition.vertical === 'both');
    if (compatible.length || options.explicitKeys?.length)
        return compatible;
    return AUTOMATION_DEFINITIONS.filter((definition) => {
        const sectionOk = !options.sections?.length || options.sections.includes(definition.category);
        const verticalOk = definition.vertical === options.vertical || definition.vertical === 'both';
        return sectionOk && verticalOk;
    }).slice(0, 12);
}
async function computeCampaignCounts(jobIds) {
    if (!jobIds.length)
        return { total: 0, queued: 0, running: 0, awaitingReview: 0, completed: 0, failed: 0, cancelled: 0 };
    ...
    for (const job of jobs) {
        ...
        else if (['succeeded', 'approved', 'published'].includes(job.status))
            counts.completed += 1;
        else if (['failed', 'dead_letter'].includes(job.status))
            counts.failed += 1;
...
function deriveCampaignStatus(counts) {
    if (!counts.total)
        return 'created';
    if (counts.failed && counts.failed === counts.total)
        return 'failed';
    if (counts.failed)
...
function normalizeSourceEntity(sourceEntity, title, sport) {
    if (typeof sourceEntity?.type === 'string' && sourceEntity.type.trim())
        return { ...sourceEntity, type: sourceEntity.type, sport: sourceEntity.sport || sport };
...
function campaignTitle(pack, sourceEntity, sport) {
...
function humanIntent(campaignType, sport) {
    if (campaignType === 'fight_tonight_campaign')
        return `Promote tonight's ${sport === 'boxing' ? 'boxing' : 'fight'} event with all safe agents.`;
    if (campaignType === 'boxing_fight_campaign')
...
function safeCampaignMode(mode, definition) {
    if (mode === 'AUTOMATED' && !definition.supportsAutoMode)
        return definition.defaultMode;
    if (mode === 'AUTOMATED' && definition.riskLevel === 'high')
