...
import type { SwarmArtifactDocument } from '../models/artifact.model.js';
...
function buildCallbackUrl(path: string, explicitUrl?: string): string | undefined {
  if (explicitUrl) return explicitUrl;
  if (!env.BACKEND_BASE_URL) return undefined;
  return `${env.BACKEND_BASE_URL.replace(/\/$/, '')}${path}`;
}

export async function sendJobCompletedCallback(job: SwarmJobDocument, artifact: SwarmArtifactDocument): Promise<void> {
  if (!env.BACKEND_CALLBACK_ENABLED) return;
...
export async function sendJobFailedCallback(job: SwarmJobDocument): Promise<void> {
  if (!env.BACKEND_CALLBACK_ENABLED) return;
...
async function postSignedJson(url: string, payload: Record<string, unknown>): Promise<void> {
