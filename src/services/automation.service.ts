import { z } from 'zod';
import { AUTOMATION_DEFINITIONS, automationByKey, automationsByTrigger, type AutomationDefinition } from '../automations/definitions.js';
import { swarmModeSchema, verticalSchema, type SwarmMode, type Vertical } from '../contracts/domain.js';
import { AutomationLog } from '../models/automation-log.model.js';
import { AutomationSetting, type AutomationSettingDocument } from '../models/automation-setting.model.js';
import { createJob } from './job.service.js';
import { AppError } from '../utils/errors.js';
import { isoDateKey } from '../utils/time.js';

export interface EffectiveAutomationSetting {
  key: string;
  enabled: boolean;
  mode: SwarmMode;
...
export interface AutomationListItem extends AutomationDefinition {
  setting: EffectiveAutomationSetting;
}
...
export type UpdateAutomationSettingInput = z.infer<typeof updateAutomationSettingSchema>;
export type TriggerAutomationEventInput = z.infer<typeof triggerAutomationEventSchema>;

function defaultSetting(definition: AutomationDefinition): EffectiveAutomationSetting {
  return {
    key: definition.key,
...
function mergeSetting(definition: AutomationDefinition, stored?: AutomationSettingDocument | null): EffectiveAutomationSetting {
...
function sanitizeMode(definition: AutomationDefinition, requestedMode: SwarmMode): SwarmMode {
  if (requestedMode === 'AUTOMATED' && !definition.supportsAutoMode) return definition.defaultMode;
  return requestedMode;
}

export async function listAutomations(query: { trigger?: string; category?: string; vertical?: string } = {}): Promise<AutomationListItem[]> {
...
export async function getAutomation(key: string): Promise<AutomationListItem> {
  const definition = automationByKey(key);
  if (!definition) throw new AppError(404, 'AUTOMATION_NOT_FOUND', `Automation ${key} was not found.`);
  const stored = await AutomationSetting.findOne({ key });
  return { ...definition, setting: mergeSetting(definition, stored) };
}

export async function updateAutomationSetting(key: string, rawInput: UpdateAutomationSettingInput): Promise<AutomationListItem> {
  const definition = automationByKey(key);
  if (!definition) throw new AppError(404, 'AUTOMATION_NOT_FOUND', `Automation ${key} was not found.`);
...
export async function bulkUpdateAutomationSettings(items: Array<{ key: string } & UpdateAutomationSettingInput>): Promise<AutomationListItem[]> {
  const updated: AutomationListItem[] = [];
...
export async function resetAutomationSetting(key: string, actor?: Record<string, unknown>): Promise<AutomationListItem> {
  const definition = automationByKey(key);
  if (!definition) throw new AppError(404, 'AUTOMATION_NOT_FOUND', `Automation ${key} was not found.`);
...
export async function getAutomationDashboard(): Promise<Record<string, unknown>> {
  const automations = await listAutomations();
  const enabled = automations.filter((item) => item.setting.enabled);
  const byCategory = automations.reduce<Record<string, number>>((acc, item) => {
    acc[item.category] = (acc[item.category] || 0) + 1;
    return acc;
  }, {});
  const enabledByCategory = enabled.reduce<Record<string, number>>((acc, item) => {
    acc[item.category] = (acc[item.category] || 0) + 1;
    return acc;
  }, {});
...
export async function triggerAutomationEvent(rawInput: TriggerAutomationEventInput): Promise<Record<string, unknown>> {
  ...
  for (const automation of automations) {
    ...
    const result = await createJob({
      ...
      idempotencyKey,
      requestedBy: { ...input.requestedBy, source: String(input.requestedBy.source || 'backend') as any },
      sourceEntity,
      input: {
        ...input.input,
        automationKey: automation.key,
...
export async function listAutomationLogs(query: { key?: string; trigger?: string; limit?: number } = {}): Promise<Record<string, unknown>[]> {
...
function inferVerticalFromTrigger(trigger: string): Vertical {
  if (trigger.includes('wrestl')) return 'pro_wrestling';
  return 'combat';
}
