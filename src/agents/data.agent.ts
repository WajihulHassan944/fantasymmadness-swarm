...
export class DataCandidateAgent implements SwarmAgent {
  readonly name = 'data-candidate-agent';
  ...
  private async eventCalendarDailyUpdate(job: SwarmJobDocument): Promise<AgentExecutionResult> {
  ...
  private async calendarRefreshPlan(job: SwarmJobDocument): Promise<AgentExecutionResult> {
    const input = job.input || {};
    const sport = getString(input, 'sport', getString(input, 'discipline', job.vertical === 'pro_wrestling' ? 'pro_wrestling' : 'mma'));
  ...
  private async trendReport(job: SwarmJobDocument): Promise<AgentExecutionResult> {
    const input = job.input || {};
  ...
  private async externalCandidate(job: SwarmJobDocument): Promise<AgentExecutionResult> {
    const input = job.input || {};
  ...
  private calendarCard(item: unknown, index: number): Record<string, unknown> {
  ...
  private sportLabel(sport: string): string {
  ...
  private normalize(job: SwarmJobDocument, rawRecord: Record<string, unknown>): Record<string, unknown> {
  ...
  private confidenceScore(rawRecord: Record<string, unknown>): number {
    const keyCount = Object.keys(rawRecord).length;
    if (keyCount >= 8) return 82;
    if (keyCount >= 4) return 68;
    return 45;
  }
}
