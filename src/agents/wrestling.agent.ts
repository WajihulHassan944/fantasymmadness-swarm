import { wrestlingStatsSchema, type WrestlingStats, type WebsiteBlogDraft } from '../contracts/domain.js';
...
const DEFAULT_STATS: WrestlingStats = { HP: 14, BP: 9, K: 5, PM: 4, FM: 1 };

type WrestlerInput = Record<string, unknown> & {
  displayName?: string;
  name?: string;
  promotion?: string;
  wrestlingStyle?: string;
  historicalStatistics?: Record<string, unknown>;
};

interface ScorecardPayload extends Record<string, unknown> {
  competitorA: WrestlingStats;
  competitorB: WrestlingStats;
  winnerPrediction: 'A' | 'B' | 'DRAW';
  confidence: number;
  rationale: string[];
  safety: {
    advisoryOnly: boolean;
    officialScoringOwner: string;
  };
}

interface WrestlerProfilePayload extends Record<string, unknown> {
  displayName: string;
  slug: string;
  promotion?: string;
  wrestlingStyle?: string;
  ...
  biography: string;
  historicalStatistics: WrestlingStats & { matches: number };
  seo: {
    title: string;
    description: string;
    keywords: string[];
...
export class WrestlingAgent implements SwarmAgent {
  readonly name = 'wrestling-agent';
  ...
  async run(job: SwarmJobDocument): Promise<AgentExecutionResult> {
    if (job.jobType === 'wrestling.scorecard-suggestion') return this.scorecardSuggestion(job);
    if (job.jobType === 'wrestling.wrestler-profile') return this.wrestlerProfile(job);
    return this.matchAnalysis(job);
  }

  private async scorecardSuggestion(job: SwarmJobDocument): Promise<AgentExecutionResult> {
    const input = job.input || {};
    const competitorA = coerceRecord(input.competitorA) as WrestlerInput;
    const competitorB = coerceRecord(input.competitorB) as WrestlerInput;
    ...
    const fallback: ScorecardPayload = {
      competitorA: statsA,
      competitorB: statsB,
    ...
    const aiResult = await ai.generateJson<ScorecardPayload>({
  ...
  private async matchAnalysis(job: SwarmJobDocument): Promise<AgentExecutionResult> {
    const input = job.input || {};
    const competitorA = coerceRecord(input.competitorA) as WrestlerInput;
    const competitorB = coerceRecord(input.competitorB) as WrestlerInput;
    const nameA = this.wrestlerName(competitorA, 'Competitor A');
    ...
    const fallback: WebsiteBlogDraft = {
      vertical: 'pro_wrestling',
  ...
  private async wrestlerProfile(job: SwarmJobDocument): Promise<AgentExecutionResult> {
    const input = job.input || {};
    const wrestler = coerceRecord(input.wrestler) as WrestlerInput;
    ...
    const fallback: WrestlerProfilePayload = {
      displayName,
    ...
    const ai = getAiProvider();
    const aiResult = await ai.generateJson<WrestlerProfilePayload>({
  ...
  private wrestlerName(wrestler: WrestlerInput, fallback: string): string {
    return String(wrestler.displayName || wrestler.name || fallback).trim();
  }

  private estimateStats(wrestler: WrestlerInput, modifier: number): WrestlingStats {
  ...
  private pickWinner(statsA: WrestlingStats, statsB: WrestlingStats): 'A' | 'B' | 'DRAW' {
    ...
    if (Math.abs(totalA - totalB) < 2) return 'DRAW';
    return totalA >= totalB ? 'A' : 'B';
  }

  private normalizeScorecard(output: ScorecardPayload, fallback: ScorecardPayload): ScorecardPayload {
