...
type SocialPlatform = 'x' | 'instagram' | 'facebook' | 'discord' | 'youtube';
...
interface SocialDraftPayload extends Record<string, unknown> {
  ...
  youtubeVideos?: Array<{
    ...
    requiredEndingLine: string;
  }>;
  shortFormClips?: Array<{
  ...
  publicationReadiness: {
    ...
    platformReadiness: Array<{ platform: SocialPlatform; configured: boolean; missing: string[] }>;
  };
}
...
export class SocialAgent implements SwarmAgent {
  ...
  async run(job: SwarmJobDocument): Promise<AgentExecutionResult> {
    ...
    if (!env.BRAND_LOGO_URL) warnings.push('BRAND_LOGO_URL is not configured; visual briefs require a small logo overlay but cannot point to a concrete logo yet.');
  ...
  private youtubeVideoDrafts(topic: string, sport = 'mma'): NonNullable<SocialDraftPayload['youtubeVideos']> {
  ...
  private shortFormClips(topic: string, sport = 'mma'): NonNullable<SocialDraftPayload['shortFormClips']> {
  ...
  private defaultCampaignName(job: SwarmJobDocument, sport = 'mma'): string {
    if (this.isGrowthJob(job.jobType)) return 'Fantasy MMadness July 10,000 Signup Growth System';
  ...
  private defaultPlatforms(jobType: JobType): string[] {
    if (jobType === 'social.youtube-growth-video-draft') return ['youtube'];
    if (jobType === 'social.short-form-video-pack') return ['youtube', 'instagram', 'facebook', 'x'];
    if (jobType === 'social.x-growth-posts') return ['x'];
    if (jobType === 'social.instagram-growth-posts') return ['instagram'];
    if (jobType === 'social.facebook-growth-posts') return ['facebook'];
    if (jobType.includes('discord')) return ['discord'];
    if (jobType.includes('youtube')) return ['youtube', 'x'];
    if (jobType.includes('instagram')) return ['instagram'];
    if (jobType.includes('facebook')) return ['facebook'];
    if (jobType.includes('multi-platform') || jobType.includes('promotional') || jobType.includes('campaign')) return env.SOCIAL_DEFAULT_PLATFORMS;
    return env.SOCIAL_DEFAULT_PLATFORMS.length ? env.SOCIAL_DEFAULT_PLATFORMS : ['x'];
  }

  private defaultPostText(jobType: JobType, topic: string, index = 0): string {
    if (jobType === 'social.x-growth-posts') return `${topic}: prediction window is open. Post ${index + 1}: pick angle, deadline reminder, or live reaction. Make your picks on Fantasy MMadness before the event starts.`;
    if (jobType === 'social.instagram-growth-posts') return `${topic} is built for predictions. Use the poster, poll, or story frame to push fans toward Fantasy MMadness picks before lock time.`;
    if (jobType === 'social.facebook-growth-posts') return `${topic} discussion: who are you backing and why? Make your picks on Fantasy MMadness before the event starts.`;
  ...
  private defaultCallToAction(jobType: JobType): string {
    if (this.isGrowthJob(jobType) || jobType.includes('youtube')) return 'Make your picks on Fantasy MMadness before the event starts.';
  ...
  private defaultMediaSuggestion(jobType: JobType, topic: string): string {
    const logoNote = this.logoNote();
    if (jobType === 'social.instagram-growth-posts') return `Instagram 4:5 or 9:16 prediction graphic for ${topic}, with poster/poll/story variant and ${logoNote}.`;
    if (jobType === 'social.facebook-growth-posts') return `Facebook discussion graphic for ${topic}, community question layout, event/prediction visual, and ${logoNote}.`;
    if (jobType === 'social.x-growth-posts') return `X-friendly fight prediction card or live-reaction graphic for ${topic}, concise text, and ${logoNote}.`;
    return `Use approved event, fighter, wrestler, contest, or blog artwork with ${logoNote}.`;
  }

  private defaultScheduledSlot(jobType: JobType, platform: SocialPlatform, index: number): string {
    if (jobType === 'social.x-growth-posts') return ['8 AM', '10 AM', '12 PM', '2 PM', '5 PM', '7 PM', '10 PM'][index % 7] || 'rolling X slot';
    if (platform === 'instagram') return ['8 AM', '5 PM', '7 PM', '10 PM'][index % 4] || 'Instagram slot';
    if (platform === 'facebook') return ['12 PM', '7 PM', '10 PM'][index % 3] || 'Facebook slot';
    if (platform === 'youtube') return ['10 AM', '2 PM', '7 PM', '10 PM'][index % 4] || 'YouTube slot';
    return 'daily publishing slot';
  }

  private contentPillarFor(jobType: JobType): string {
    if (jobType.includes('youtube')) return 'long_form_trust_and_discovery';
    if (jobType.includes('short-form')) return 'mass_reach_short_form';
    if (jobType.includes('instagram')) return 'fast_awareness';
    if (jobType.includes('facebook')) return 'community_engagement';
    if (jobType.includes('x-growth')) return 'real_time_visibility';
    return 'content_to_prediction_to_signup';
  }

  private postCountFor(jobType: JobType, platform?: SocialPlatform): number {
    if (jobType === 'social.instagram-growth-posts') return Math.max(4, Math.min(6, env.GROWTH_DAILY_INSTAGRAM_POSTS));
    if (jobType === 'social.facebook-growth-posts') return Math.max(3, Math.min(5, env.GROWTH_DAILY_FACEBOOK_POSTS));
    if (jobType === 'social.x-growth-posts') return Math.max(8, Math.min(15, env.GROWTH_DAILY_X_POSTS));
    if (jobType === 'social.multi-platform-daily-posts') {
      if (platform === 'instagram') return Math.max(4, Math.min(6, env.GROWTH_DAILY_INSTAGRAM_POSTS));
      if (platform === 'facebook') return Math.max(3, Math.min(5, env.GROWTH_DAILY_FACEBOOK_POSTS));
      if (platform === 'x') return Math.max(8, Math.min(15, env.GROWTH_DAILY_X_POSTS));
    }
    return 1;
  }

  private hashtags(vertical: string, sport: string, extra: string[], defaults: string[] = []): string[] {
    return [...new Set(['FantasyMMAdness', this.defaultSportHashtag(vertical, sport), ...defaults, ...extra])];
  }

  private dailyTargets(): Record<string, unknown> {
    return {
  ...
  private brandOverlay(): NonNullable<SocialDraftPayload['brandOverlay']> {
    return {
      required: true,
  ...
  private requiredYouTubeEndingLine(): string {
    return 'Make your picks on Fantasy MMadness before the event starts.';
  }

  private safetyNotes(jobType: JobType): string[] {
    const notes = [
    ...
    if (jobType.includes('youtube')) notes.push(`Every YouTube video must end with: "${this.requiredYouTubeEndingLine()}"`);
    return notes;
  }

  private isGrowthJob(jobType: JobType): boolean {
    return jobType.includes('growth') || jobType.includes('short-form');
  }

  private sportLabel(sport = 'mma'): string {
    if (sport === 'boxing') return 'boxing';
    if (sport === 'kickboxing') return 'kickboxing';
    if (sport === 'pro_wrestling') return 'pro-wrestling';
    return 'combat sports';
  }

  private normalizePlatforms(platforms: string[]): SocialPlatform[] {
    const allowed: SocialPlatform[] = ['x', 'instagram', 'facebook', 'discord', 'youtube'];
    return [...new Set(platforms.map((platform) => platform.toLowerCase().trim()).filter((platform): platform is SocialPlatform => allowed.includes(platform as SocialPlatform)))];
  }

  private platformReadiness(platform: SocialPlatform): { platform: SocialPlatform; configured: boolean; missing: string[] } {
    const missing: string[] = [];
    if (platform === 'x') {
      if (!env.TWITTER_API_KEY) missing.push('TWITTER_API_KEY');
      if (!env.TWITTER_API_SECRET) missing.push('TWITTER_API_SECRET');
      if (!env.TWITTER_ACCESS_TOKEN) missing.push('TWITTER_ACCESS_TOKEN');
      if (!env.TWITTER_ACCESS_SECRET) missing.push('TWITTER_ACCESS_SECRET');
    }
    ...
    if (platform === 'instagram') {
      ...
      if (!env.INSTAGRAM_ACCESS_TOKEN && !env.FACEBOOK_PAGE_ACCESS_TOKEN) missing.push('INSTAGRAM_ACCESS_TOKEN or FACEBOOK_PAGE_ACCESS_TOKEN');
    }
    if (platform === 'discord') missing.push('DISCORD_WEBHOOK_URL');
    if (platform === 'youtube') {
      if (!env.YOUTUBE_CHANNEL_ID) missing.push('YOUTUBE_CHANNEL_ID');
