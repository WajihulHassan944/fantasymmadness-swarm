...
export class SeoAgent implements SwarmAgent {
  ...
  private artifactTypeFor(jobType: JobType) {
    if (jobType === 'seo.metadata' || jobType === 'seo.opengraph-twitter-cards' || jobType === 'seo.fighter-refresh') return 'seo.metadata-package' as const;
    if (jobType === 'seo.schema-markup' || jobType === 'seo.fight-event-structured-data' || jobType === 'seo.fighter-wrestler-structured-data') return 'seo.schema-markup' as const;
    if (jobType === 'seo.sitemap-refresh') return 'seo.sitemap-refresh-plan' as const;
    if (jobType === 'seo.internal-links' || jobType === 'seo.related-post-linking') return 'seo.internal-link-plan' as const;
    if (jobType === 'seo.keyword-opportunity') return 'seo.keyword-opportunity-report' as const;
    if (jobType !== 'seo.audit' && jobType !== 'seo.blog-audit' && jobType !== 'seo.daily-audit' && jobType !== 'seo.weekly-traffic-opportunity') return 'seo.technical-issue-report' as const;
  ...
  private titlePrefix(jobType: JobType): string {
    ...
    if (jobType.includes('sitemap')) return 'Sitemap plan';
    if (jobType.includes('link')) return 'Internal link plan';
